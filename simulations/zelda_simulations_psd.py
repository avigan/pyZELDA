...
# characteristics of the opd map
Dpup       = opd.shape[-1]
...
# ----------------------------------------
# psd computation and integration
# ----------------------------------------
# with fft
...
# ----------------------------------------
# estimation of the wavefront errors at different spatial frequency regimes
# ----------------------------------------
# bounds of the spatial frequency ranges for wavefront error estimation
freq0 = 0.
freq1 = 4.
freq2 = 20.
freq3 = 384

# estimation from fft computation
...
# estimation from mft computation
...
# comparison between total wavefront error from sum of terms and from psd integration (from fft)
sigma_from_psd_mft = np.sqrt(sigma1_mft**2+sigma2_mft**2+sigma3_mft**2)
print('total std mft: {0:2f}'.format(sigma_from_psd_mft))
sigma_from_psd_fft = np.sqrt(sigma1_fft**2+sigma2_fft**2+sigma3_fft**2)
print('total std fft: {0:2f}'.format(sigma_from_psd_fft))
